...
protocol::PlanConversionResponse prestoToBoltPlanConversion(
  ...
  try {
    ...
    protocol::TaskId taskId = "bolt-plan-conversion.0.0.0.0";
    auto tableWriteInfo = std::make_shared<protocol::TableWriteInfo>();

    // Attempt to convert the plan fragment to a Bolt plan.
    if (auto writeNode =
      ...
      auto writeSourceNode =
          converter.toBoltQueryPlan(writeNode->source, tableWriteInfo, taskId);
      planValidator->validatePlanFragment(core::PlanFragment(writeSourceNode));
    } else {
      auto boltPlan =
          converter.toBoltQueryPlan(planFragment, tableWriteInfo, taskId);
      planValidator->validatePlanFragment(boltPlan);
    }
  } catch (const BoltException& e) {
    response.failures.emplace_back(
        copyFailureInfo(BoltToPrestoExceptionTranslator::translate(e)));
